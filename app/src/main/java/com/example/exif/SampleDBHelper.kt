...
class SampleDBHelper(
    ...
    override fun onCreate(database: SQLiteDatabase?) {
        ...
        database?.execSQL("create table if not exists Meta (photo_id text primary key, image_name text, image_length text, image_width text, bits_per_sample text, compression text, image_description text, image_orientation text, maker text, model text, strip_offsets text, gps_version_id text, gps_latitude text, gps_longitude text, date_time_original text, change_date_and_time text, FOREIGN KEY(photo_id) REFERENCES Photo(id))");
    }
