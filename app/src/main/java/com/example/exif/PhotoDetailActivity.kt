...
var allImageSize: Array<Long?> = emptyArray()
...
var allImageOcr: Array<String?> = emptyArray()
...
//        val SizeStr: String = Formatter.formatFileSize(this, imageSize!!)
        ...
        val sqlImageSize = "SELECT size FROM Photo"
        ...
        val sqlOcr = "SELECT ocr FROM Photo"
        ...
        val cursorImageSize = databaseR.rawQuery(sqlImageSize, null)
        ...
        val cursorOcr = databaseR.rawQuery(sqlOcr, null)
        ...
        try {
            cursorImageSize.moveToFirst()
            allImageSize = arrayOfNulls(cursorImageSize.count)
            for (i in allImageSize.indices) {
                allImageSize[i]= cursorImageSize.getLong(0)
                cursorImageSize.moveToNext()
            }
            cursorImageSize.close()
        } catch (e: Exception) {
            e.printStackTrace()
        }
        ...
        try {
            cursorOcr.moveToFirst()
            allImageOcr = arrayOfNulls(cursorOcr.count)
            for (i in allImageOcr.indices) {
                allImageOcr[i] = cursorOcr.getString(0)
                cursorOcr.moveToNext()
            }
            cursorOcr.close()
        } catch (e: Exception) {
            e.printStackTrace()
